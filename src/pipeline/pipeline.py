...
def call_scantist_SCA(repo_path, bom_detector_path, serverUrl="http://119.8.181.73:8237/"):
    ...
    """
    p = subprocess.run(f"java -jar {bom_detector_path} -working_dir {repo_path} -download_report -serverUrl {serverUrl}"
                       f"--debug")
    if p.returncode != 0:
        raise SystemError(p.stderr)
...
def process_repository(repo_str):
    ...
    tqdm.write('checking if the scantist-bom-detector is already downlaoded')
    did_download, bom_detector_path = download_scantist_bom_detector()
    if did_download:
